/**
 * Levi's Stadium menu cleanup heuristics.
 *
 * Official concessions import + NFL league import overlap on concept-level
 * vendor rows (name = vendor). Hide stand placeholders and NFL non-food rows;
 * keep headline named dishes and Bay Area / 49ers specialties.
 */
...
export const LEVI_S_STADIUM_VENUE_SLUG = "levi-s-stadium";

export type LeviSStadiumCleanupAction = MenuQualityAction;

/** Headline / named specialty rows where item title matches vendor — keep active. */
export const LEVI_S_STADIUM_CANONICAL_STAND_DISH_SLUGS = new Set([
...
export const LEVI_S_STADIUM_REDUNDANT_STAND_LABEL_SLUGS = new Set([
  "chicken-guy-intelligent-stadium-tech",
  "chicken-guy-the-largest-4k-displays-in-the-nfl",
  "incogmeato-by-morningstar-farms-free-filtered-water"
]);

export type LeviSStadiumCuratedGroupSpec = {
  id: string;
  label: string;
...
export const LEVI_S_STADIUM_CURATED_CLEANUP_GROUPS: LeviSStadiumCuratedGroupSpec[] =
  [];

export const LEVI_S_STADIUM_GENERIC_CONCESSION_SLUGS = new Set<string>([]);
...
const NFL_NON_FOOD_RE =
  /\b(4k display|intelligent stadium|wifi|5g das|video board|cellular network)\b/i;

const VENDOR_STAND_EXEMPT_SLUGS = new Set([
  ...LEVI_S_STADIUM_CANONICAL_STAND_DISH_SLUGS
]);

export function isLeviSStadiumVendorStandPlaceholder(item: {
  slug: string;
  name: string;
  vendor: { name: string };
}): boolean {
...
export function isLeviSStadiumGenericConcessionRow(slug: string): boolean {
  return LEVI_S_STADIUM_GENERIC_CONCESSION_SLUGS.has(slug);
}

export function isLeviSStadiumGenericBeverageRow(name: string): boolean {
  return GENERIC_BEVERAGE_RE.test(name.trim());
}

export function isLeviSStadiumVaguePlaceholderRow(name: string): boolean {
  return (
    VAGUE_PLACEHOLDER_RE.test(name.trim()) || NFL_NON_FOOD_RE.test(name.trim())
  );
}

export function getLeviSStadiumKeepBothMemberIds<
...
export type LeviSStadiumRowClassification =
...
export function classifyLeviSStadiumRow(
  item: {
    slug: string;
    name: string;
    vendor: { name: string };
  ...
  if (LEVI_S_STADIUM_REDUNDANT_STAND_LABEL_SLUGS.has(item.slug)) {
    return {
      kind: "vendor-only",
  ...
  if (isLeviSStadiumVendorStandPlaceholder(item)) {
    return {
      kind: "vendor-only",
      action: "hide-duplicate",
      reason:
  ...
  if (isLeviSStadiumGenericConcessionRow(item.slug)) {
    return {
  ...
  if (isLeviSStadiumGenericBeverageRow(item.name)) {
    return {
  ...
  if (isLeviSStadiumVaguePlaceholderRow(item.name)) {
    return {
      kind: "vague",
      action: "hide-generic",
...
export function buildLeviSStadiumCuratedGroups<
  T extends {
    id: string;
    name: string;
    vendor: { name: string };
...
>(items: T[]) {
  const groups: {
    spec: LeviSStadiumCuratedGroupSpec;
    members: T[];
  }[] = [];
...
export function pickLeviSStadiumCuratedKeep<T extends MenuQualityItem>(
  members: T[],
  spec: LeviSStadiumCuratedGroupSpec
): T {
