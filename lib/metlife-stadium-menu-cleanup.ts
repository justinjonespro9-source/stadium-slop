/**
 * MetLife Stadium menu cleanup heuristics.
 *
 * Official dining import + NFL league import overlap on concept-level vendor
 * rows (name = vendor). Hide stand placeholders and NFL non-menu rows; keep
 * headline named dishes and NY/NJ Giants/Jets specialties.
 */
...
export const METLIFE_STADIUM_VENUE_SLUG = "metlife-stadium";

export type MetlifeStadiumCleanupAction = MenuQualityAction;

/** Headline / named specialty rows where item title matches vendor — keep active. */
export const METLIFE_STADIUM_CANONICAL_STAND_DISH_SLUGS = new Set([
...
export const METLIFE_STADIUM_REDUNDANT_STAND_LABEL_SLUGS = new Set([
  "a-cut-above-by-against-the-grain-jets-games"
]);

export type MetlifeStadiumCuratedGroupSpec = {
  id: string;
  label: string;
...
export const METLIFE_STADIUM_CURATED_CLEANUP_GROUPS: MetlifeStadiumCuratedGroupSpec[] =
  [];

export const METLIFE_STADIUM_GENERIC_CONCESSION_SLUGS = new Set<string>([]);
...
const VAGUE_PLACEHOLDER_RE =
  /^(assorted snacks?|grab[- ]and[- ]go snacks?|beverage package|all[- ]you[- ]can[- ]eat package|premium buffet|rotating carved sandwich|member inclusive menu|jets games|giants games)$/i;

const NFL_NON_FOOD_RE = /\b(jets games|giants games|uber eats|zippin)\b/i;

const VENDOR_STAND_EXEMPT_SLUGS = new Set([
  ...METLIFE_STADIUM_CANONICAL_STAND_DISH_SLUGS
]);

export function isMetlifeStadiumVendorStandPlaceholder(item: {
  slug: string;
  name: string;
  vendor: { name: string };
}): boolean {
...
export function isMetlifeStadiumGenericConcessionRow(slug: string): boolean {
  return METLIFE_STADIUM_GENERIC_CONCESSION_SLUGS.has(slug);
}

export function isMetlifeStadiumGenericBeverageRow(name: string): boolean {
  return GENERIC_BEVERAGE_RE.test(name.trim());
}

export function isMetlifeStadiumVaguePlaceholderRow(name: string): boolean {
  return (
    VAGUE_PLACEHOLDER_RE.test(name.trim()) || NFL_NON_FOOD_RE.test(name.trim())
  );
}

export function getMetlifeStadiumKeepBothMemberIds<
...
export type MetlifeStadiumRowClassification =
...
export function classifyMetlifeStadiumRow(
  item: {
    slug: string;
    name: string;
    vendor: { name: string };
...
): MetlifeStadiumRowClassification {
  if (
  ...
  if (METLIFE_STADIUM_REDUNDANT_STAND_LABEL_SLUGS.has(item.slug)) {
    return {
      kind: "vendor-only",
  ...
  if (isMetlifeStadiumVendorStandPlaceholder(item)) {
    return {
      kind: "vendor-only",
      action: "hide-duplicate",
      reason:
  ...
  if (isMetlifeStadiumGenericConcessionRow(item.slug)) {
    return {
  ...
  if (isMetlifeStadiumGenericBeverageRow(item.name)) {
    return {
  ...
  if (isMetlifeStadiumVaguePlaceholderRow(item.name)) {
    return {
      kind: "vague",
      action: "hide-generic",
...
export function buildMetlifeStadiumCuratedGroups<
  T extends {
    id: string;
    name: string;
    vendor: { name: string };
...
>(items: T[]) {
  const groups: {
    spec: MetlifeStadiumCuratedGroupSpec;
    members: T[];
  }[] = [];
...
export function pickMetlifeStadiumCuratedKeep<T extends MenuQualityItem>(
  members: T[],
  spec: MetlifeStadiumCuratedGroupSpec
): T {
