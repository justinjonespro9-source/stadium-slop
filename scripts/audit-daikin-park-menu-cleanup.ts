#!/usr/bin/env npx tsx
/**
 * Daikin Park menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-daikin-park-menu-cleanup.ts
 *   npm run audit:daikin-park-menu
 *   npx tsx scripts/audit-daikin-park-menu-cleanup.ts --apply
 */
...
import {
  buildDaikinCuratedGroups,
  classifyDaikinParkRow,
  isDaikinVendorStandPlaceholder,
  DAIKIN_CURATED_CLEANUP_GROUPS,
  DAIKIN_PARK_VENUE_SLUG,
  recommendCuratedMemberAction,
  type DaikinCleanupAction
} from "../lib/daikin-park-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: DaikinCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildDaikinCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildDaikinCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: DAIKIN_PARK_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${DAIKIN_PARK_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildDaikinCuratedDuplicateGroups(items);

  const vendorOnlyRows = active.filter((item) => isDaikinVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyDaikinParkRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyDaikinParkRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyDaikinParkRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: DaikinCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyDaikinParkRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    for (const member of group.members) {
      const spec = DAIKIN_CURATED_CLEANUP_GROUPS.find((s) => s.id === group.key)!;
  ...
  console.log(
    dryRun ? "DAIKIN PARK MENU CLEANUP AUDIT (DRY RUN)" : "DAIKIN PARK MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${DAIKIN_PARK_VENUE_SLUG})`);
  ...
  console.log(
    "  KEEP (canonical): Brisket Donuts, Crawlache, Maple Brisket Wafflewich, Whataburger — name=vendor specialties."
  );
  console.log(
    "  KEEP BOTH: Frozen Margarita at El Tiempo Cantina and El Tiempo Margarita Bar."
  );
  ...
  console.log(
    "  · lib/venue-menu-import/daikin-park.ts imports vendor concepts as FoodItem names."
  );
  console.log(
    "  · Expansion seed (lib/mlb-ballpark-concessions-parks-wave-4.ts) adds named dishes."
  );
  console.log(
    "  · Brisket Donuts, Crawlache, Banh Mi Dog, and Boomin Onion items kept despite broad-audit flags."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-daikin-park-menu-cleanup.ts --apply");
    return;
  }
