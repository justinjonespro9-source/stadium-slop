#!/usr/bin/env npx tsx
/**
 * Gillette Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-gillette-stadium-menu-cleanup.ts
 *   npm run audit:gillette-stadium-menu
 *   npx tsx scripts/audit-gillette-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildGilletteStadiumCuratedGroups,
  classifyGilletteStadiumRow,
  getGilletteStadiumKeepBothMemberIds,
  isGilletteStadiumVendorStandPlaceholder,
  GILLETTE_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  GILLETTE_STADIUM_CURATED_CLEANUP_GROUPS,
  GILLETTE_STADIUM_VENUE_SLUG,
  pickGilletteStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type GilletteStadiumCleanupAction
} from "../lib/gillette-stadium-menu-cleanup";
import {
...
const SOURCE_URL =
  "https://www.gillettestadium.com/concessions/";
...
function formatItemLine(item: ItemRow, action: GilletteStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildGilletteStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildGilletteStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: GILLETTE_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${GILLETTE_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getGilletteStadiumKeepBothMemberIds(active);
  ...
  const curatedGroups = buildGilletteStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isGilletteStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyGilletteStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyGilletteStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyGilletteStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: GilletteStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyGilletteStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickGilletteStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickGilletteStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    ...
    "beacon-bbq-beacon-bbq-platter",
    "market-express-the-dirty-patriot-soda",
    "takeaway-tavern-drum-sauce-fried-chicken-wings",
    "doms-steak-tip-sub",
    "beef-brisket-sandwich",
    "pulled-pork-sandwich",
    "pulled-pork-nachos",
    "the-landing-bar-international-empanada-flight",
    "the-landing-bar-halal-friendly-chicken-skewers",
    "hand-breaded-fried-chicken-sandwich",
    "smashburger",
    "bavarian-pretzel-log",
    "steak-and-cheese-sub"
  ]);

  const specialtyDrinkRows = active.filter(
    (item) =>
      /patriot soda|dirty.*soda/i.test(`${item.name} ${item.slug}`) &&
      !hidePlan.has(item.id)
  );

  console.log("═".repeat(72));
  console.log(
    dryRun
      ? "GILLETTE STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "GILLETTE STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${GILLETTE_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of GILLETTE_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickGilletteStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickGilletteStadiumCuratedKeep(
                    members,
                    GILLETTE_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (New England / Patriots / Revolution / World Cup specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      GILLETTE_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (Dirty Patriot Soda) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Named cocktail headline row (Dirty Patriot Soda) is kept as a canonical stand dish."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/gillette-stadium.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline items (Clam Chowder Poutine, Dirty Patriot Soda) kept where name=vendor."
  );
  console.log(
    "  · Hide stand placeholders (Dom's Steak Tips, Papa Gino's, Cisco Brewers, etc.) and NFL junk rows ($5 Value Staples, Gluten-Free Awareness, DraftKings Sports Zone)."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-gillette-stadium-menu-cleanup.ts --apply");
    return;
  }
