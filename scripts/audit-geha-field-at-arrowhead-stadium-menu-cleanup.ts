#!/usr/bin/env npx tsx
/**
 * GEHA Field at Arrowhead Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-geha-field-at-arrowhead-stadium-menu-cleanup.ts
 *   npm run audit:geha-field-menu
 *   npx tsx scripts/audit-geha-field-at-arrowhead-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildGehaFieldAtArrowheadStadiumCuratedGroups,
  classifyGehaFieldAtArrowheadStadiumRow,
  getGehaFieldAtArrowheadStadiumKeepBothMemberIds,
  isGehaFieldAtArrowheadStadiumVendorStandPlaceholder,
  GEHA_FIELD_AT_ARROWHEAD_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  GEHA_FIELD_AT_ARROWHEAD_STADIUM_CURATED_CLEANUP_GROUPS,
  GEHA_FIELD_AT_ARROWHEAD_STADIUM_VENUE_SLUG,
  pickGehaFieldAtArrowheadStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type GehaFieldAtArrowheadStadiumCleanupAction
} from "../lib/geha-field-at-arrowhead-stadium-menu-cleanup";
import {
...
const SOURCE_URL =
  "https://www.gehafieldatarrowhead.com/amenities";
...
function formatItemLine(item: ItemRow, action: GehaFieldAtArrowheadStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildGehaFieldAtArrowheadStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildGehaFieldAtArrowheadStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: GEHA_FIELD_AT_ARROWHEAD_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${GEHA_FIELD_AT_ARROWHEAD_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getGehaFieldAtArrowheadStadiumKeepBothMemberIds(active);
  ...
  const curatedGroups = buildGehaFieldAtArrowheadStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isGehaFieldAtArrowheadStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyGehaFieldAtArrowheadStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyGehaFieldAtArrowheadStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyGehaFieldAtArrowheadStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: GehaFieldAtArrowheadStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyGehaFieldAtArrowheadStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickGehaFieldAtArrowheadStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickGehaFieldAtArrowheadStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    ...
    "italian-stallion",
    "sheridans-frozen-custard-concrete",
  ...
  const specialtyDrinkRows = active.filter((item) =>
    /chiefs kingdom margarita|kansas city mule|dirty soda/i.test(`${item.name} ${item.slug}`)
  );

  console.log("═".repeat(72));
  console.log(
    dryRun
      ? "GEHA FIELD AT ARROWHEAD STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "GEHA FIELD AT ARROWHEAD STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${GEHA_FIELD_AT_ARROWHEAD_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of GEHA_FIELD_AT_ARROWHEAD_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickGehaFieldAtArrowheadStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickGehaFieldAtArrowheadStadiumCuratedKeep(
                    members,
                    GEHA_FIELD_AT_ARROWHEAD_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (Kansas City / Chiefs / Arrowhead specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      GEHA_FIELD_AT_ARROWHEAD_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (Chiefs Kingdom Margarita / KC Mule) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Not present as ACTIVE FoodItems. KC Cocktails / World Stage Bar rows are stand placeholders; named cocktails live in NFL import copy only."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/arrowhead-stadium.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline dishes (Loudest Mac, Catfish Po' Boy, Fluffy Tacos, Elote, Funnel Cake Apple Pie) kept despite name=vendor."
  );
  console.log(
    "  · Crabfries with Cheese — keep Chickie and Pete's brand-of-record; hide Connected Concepts operator/import spelling variant."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-geha-field-at-arrowhead-stadium-menu-cleanup.ts --apply");
    return;
  }
