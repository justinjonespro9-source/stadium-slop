#!/usr/bin/env npx tsx
/**
 * Nationals Park menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-nationals-park-menu-cleanup.ts
 *   npm run audit:nationals-park-menu
 *   npx tsx scripts/audit-nationals-park-menu-cleanup.ts --apply
 */
...
import {
  buildNationalsCuratedGroups,
  classifyNationalsParkRow,
  isNationalsVendorStandPlaceholder,
  NATIONALS_CURATED_CLEANUP_GROUPS,
  NATIONALS_PARK_VENUE_SLUG,
  recommendCuratedMemberAction,
  type NationalsCleanupAction
} from "../lib/nationals-park-menu-cleanup";
...
function formatItemLine(item: ItemRow, action: NationalsCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildNationalsCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildNationalsCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: NATIONALS_PARK_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${NATIONALS_PARK_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildNationalsCuratedDuplicateGroups(items);

  const vendorOnlyRows = active.filter((item) =>
    isNationalsVendorStandPlaceholder(item)
  );
  const genericBeverageRows = active.filter(
    (item) => classifyNationalsParkRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter(
    (item) => classifyNationalsParkRow(item).kind === "vague"
  );
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: NationalsCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyNationalsParkRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    for (const member of group.members) {
      const action = recommendCuratedMemberAction(member, keep, {
        id: group.key,
        label: group.label ?? group.key,
        canonicalName: group.preferredCanonical ?? keep.name,
        notes: group.notes ?? "",
  ...
  console.log(
    dryRun ? "NATIONALS PARK MENU CLEANUP AUDIT (DRY RUN)" : "NATIONALS PARK MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${NATIONALS_PARK_VENUE_SLUG})`);
  console.log(`Source: https://www.mlb.com/nationals/ballpark/concessions-guide`);
  console.log(`Total rows: ${items.length} · ACTIVE before: ${activeBefore}`);
  console.log(
    `Broad audit false positives (name=vendor): ${broadGenericFalsePositives.length} — reclassified below\n`
  );
  ...
  console.log("  KEEP: Capitol Slugger (nats-capitol-slugger-dog) — named half-smoke specialty.");
  ...
  console.log(`Duplicate groups (auto): ${autoGroups.length}`);
  console.log(`Curated groups: ${curatedGroups.length}`);
  ...
  console.log(
    "  · lib/venue-menu-import/nationals-park.ts imports stand concepts as FoodItem names."
  );
  console.log(
    "  · Prefer wave-2 named items; hide stand-label rows when a named sibling exists or stand is not reviewable alone."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-nationals-park-menu-cleanup.ts --apply");
    return;
  }
