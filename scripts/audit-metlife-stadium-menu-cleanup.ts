#!/usr/bin/env npx tsx
/**
 * MetLife Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-metlife-stadium-menu-cleanup.ts
 *   npm run audit:metlife-stadium-menu
 *   npx tsx scripts/audit-metlife-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildMetlifeStadiumCuratedGroups,
  classifyMetlifeStadiumRow,
  getMetlifeStadiumKeepBothMemberIds,
  isMetlifeStadiumVendorStandPlaceholder,
  METLIFE_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  METLIFE_STADIUM_CURATED_CLEANUP_GROUPS,
  METLIFE_STADIUM_VENUE_SLUG,
  pickMetlifeStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type MetlifeStadiumCleanupAction
} from "../lib/metlife-stadium-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: MetlifeStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildMetlifeStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildMetlifeStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: METLIFE_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${METLIFE_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getMetlifeStadiumKeepBothMemberIds(active);
  ...
  const curatedGroups = buildMetlifeStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isMetlifeStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyMetlifeStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyMetlifeStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyMetlifeStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: MetlifeStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyMetlifeStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickMetlifeStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickMetlifeStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    "the-final-ribeye-sandwich-the-final-ribeye-sandwich",
    "the-dirty-garden-state-soda-the-dirty-garden-state-soda",
    "mr-tot-burnt-ends-chili-tots",
    "nonna-s-spicy-rigatoni-vodka",
    "fat-rooster-hot-honey-chicken-zeppoles",
    ...
    "corned-beef-hoagie",
    "meatball-sandwich",
    "shah-s-halal-thumann-s-hot-dogs",
    "nacho-grande",
    "loaded-tots"
  ]);
  ...
  const specialtyDrinkRows = active.filter(
    (item) =>
      /garden state soda|dirty.*soda/i.test(`${item.name} ${item.slug}`) &&
  ...
  console.log(
    dryRun
      ? "METLIFE STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "METLIFE STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${METLIFE_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of METLIFE_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickMetlifeStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickMetlifeStadiumCuratedKeep(
                    members,
                    METLIFE_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (New York / New Jersey / Giants / Jets / World Cup specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      METLIFE_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (Dirty Garden State Soda) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Named cocktail headline row (Dirty Garden State Soda) is kept as a canonical stand dish."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/metlife-stadium.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline items (Final Ribeye Sandwich, Dirty Garden State Soda) kept where name=vendor."
  );
  console.log(
    "  · Hide stand placeholders (Fuku, Shah's Halal, Petite Greens, etc.) and NFL Jets Games copy row."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-metlife-stadium-menu-cleanup.ts --apply");
    return;
  }
