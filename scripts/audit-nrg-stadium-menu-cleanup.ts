#!/usr/bin/env npx tsx
/**
 * NRG Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-nrg-stadium-menu-cleanup.ts
 *   npm run audit:nrg-stadium-menu
 *   npx tsx scripts/audit-nrg-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildNrgStadiumCuratedGroups,
  classifyNrgStadiumRow,
  getNrgStadiumKeepBothMemberIds,
  isNrgStadiumVendorStandPlaceholder,
  NRG_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  NRG_STADIUM_CURATED_CLEANUP_GROUPS,
  NRG_STADIUM_VENUE_SLUG,
  pickNrgStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type NrgStadiumCleanupAction
} from "../lib/nrg-stadium-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: NrgStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildNrgStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildNrgStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: NRG_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${NRG_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getNrgStadiumKeepBothMemberIds(active);
  ...
  const curatedGroups = buildNrgStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isNrgStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyNrgStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyNrgStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyNrgStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: NrgStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyNrgStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickNrgStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickNrgStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    "trill-burgers-the-trill-town-loaded-fries",
    "og-trill-burger",
    "vegan-og-burger-vegan-og-burger",
    "truth-bbq-brisket-tallow-popcorn",
    ...
    "truth-sausage",
    "banana-pudding",
    "sizzling-fajita-tacos",
    "fully-loaded-nachos",
    "stuff-d-wings-seafood-boudin-wings",
    "craft-pita-mediterranean-nachos",
    "hugo-s-the-hugorita-float",
    ...
    "hula-hog-nachos",
    "most-valuable-porky",
    "wagyu-chopped-cheesesteak"
  ]);

  const specialtyDrinkRows = active.filter(
    (item) =>
      /h-?town soda|hugorita|frozen margarita/i.test(
        `${item.name} ${item.slug}`
  ...
  console.log(
    dryRun
      ? "NRG STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "NRG STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${NRG_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of NRG_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickNrgStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickNrgStadiumCuratedKeep(
                    members,
                    NRG_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (Houston / Texans / World Cup specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      NRG_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (Dirty H-Town Soda) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Named cocktail headline row (Dirty H-Town Soda) is kept as a canonical stand dish."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/nrg-stadium.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline items (Trill Town Loaded Fries, Dirty H-Town Soda, Vegan OG Burger) kept where name=vendor."
  );
  console.log(
    '  · Trill Town Loaded Fries — keep headline row; hide curated import duplicate at Trill Burgers.'
  );
  console.log(
    "  · Hide stand placeholders (Truth BBQ, El Tiempo, Trill Burgers stand row, etc.) and NFL junk rows (BMW Ballroom, Hugo's club row, Halal awareness copy)."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-nrg-stadium-menu-cleanup.ts --apply");
    return;
  }
