#!/usr/bin/env npx tsx
/**
 * Levi's Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-levi-s-stadium-menu-cleanup.ts
 *   npm run audit:levi-s-stadium-menu
 *   npx tsx scripts/audit-levi-s-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildLeviSStadiumCuratedGroups,
  classifyLeviSStadiumRow,
  getLeviSStadiumKeepBothMemberIds,
  isLeviSStadiumVendorStandPlaceholder,
  LEVI_S_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  LEVI_S_STADIUM_CURATED_CLEANUP_GROUPS,
  LEVI_S_STADIUM_VENUE_SLUG,
  pickLeviSStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type LeviSStadiumCleanupAction
} from "../lib/levi-s-stadium-menu-cleanup";
import {
...
const SOURCE_URL =
  "https://levisstadium.com/concessions/";
...
function formatItemLine(item: ItemRow, action: LeviSStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildLeviSStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildLeviSStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: LEVI_S_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${LEVI_S_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getLeviSStadiumKeepBothMemberIds(active);
  ...
  const curatedGroups = buildLeviSStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isLeviSStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyLeviSStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyLeviSStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyLeviSStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: LeviSStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyLeviSStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickLeviSStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickLeviSStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    "crab-sammies-crab-sammies",
    "gilroy-garlic-steak-frites-gilroy-garlic-steak-frites",
    "fog-city-frozen-irish-coffee-fog-city-frozen-irish-coffee",
    "karl-the-fog-karl-the-fog",
    "tostitos-walking-taco",
    "state-fair-sweets-the-san-fran-sticky-roll",
    "original-king-ramen",
    "vegetarian-king-ramen",
    "quesabirria-taco",
    "plant-based-brat",
    "vegan-tenders",
    "garlic-fries",
    "hella-fries",
    "tostitos-specialty-nacho",
    "chicken-guy-classic-sandwich",
    "chicken-guy-tenders",
  ...
  const specialtyDrinkRows = active.filter((item) =>
    /karl the fog|fog city frozen irish coffee|red faithful margarita|double gold/i.test(
      `${item.name} ${item.slug}`
    )
  );
  ...
  console.log(
    dryRun
      ? "LEVI'S STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "LEVI'S STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${LEVI_S_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of LEVI_S_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickLeviSStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickLeviSStadiumCuratedKeep(
                    members,
                    LEVI_S_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (Bay Area / 49ers / World Cup specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      LEVI_S_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (Karl the Fog / Fog City Frozen Irish Coffee) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Named cocktail headline rows (Karl the Fog, Fog City Frozen Irish Coffee) are kept as canonical stand dishes."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/levis-stadium.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline dishes (Crab Sammies, Gilroy Garlic Steak Frites, Fog City, Karl the Fog, Tostitos Walking Taco) kept despite name=vendor."
  );
  console.log(
    "  · Hide NFL non-food rows (4K displays, Intelligent Stadium tech, Free Filtered Water) and stand placeholders where named menu items exist."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-levi-s-stadium-menu-cleanup.ts --apply");
    return;
  }
