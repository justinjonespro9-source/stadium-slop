#!/usr/bin/env npx tsx
/**
 * Mercedes-Benz Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-mercedes-benz-stadium-menu-cleanup.ts
 *   npm run audit:mercedes-benz-stadium-menu
 *   npx tsx scripts/audit-mercedes-benz-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildMercedesBenzStadiumCuratedGroups,
  classifyMercedesBenzStadiumRow,
  isMercedesBenzStadiumVendorStandPlaceholder,
  MERCEDES_BENZ_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  MERCEDES_BENZ_STADIUM_CURATED_CLEANUP_GROUPS,
  MERCEDES_BENZ_STADIUM_VENUE_SLUG,
  pickMercedesBenzStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type MercedesBenzStadiumCleanupAction
} from "../lib/mercedes-benz-stadium-menu-cleanup";
import {
...
const SOURCE_URL =
  "https://www.mercedesbenzstadium.com/all-vendors";
...
function formatItemLine(item: ItemRow, action: MercedesBenzStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildMercedesBenzStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildMercedesBenzStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: MERCEDES_BENZ_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${MERCEDES_BENZ_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildMercedesBenzStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isMercedesBenzStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyMercedesBenzStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyMercedesBenzStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyMercedesBenzStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: MercedesBenzStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyMercedesBenzStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickMercedesBenzStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickMercedesBenzStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    ...
    "atl-nachos",
    "4-rivers-bbq-nachos",
    "4-rivers-pulled-pork-sandwich",
    "big-tex-sandwich",
    "burnt-end-nachos",
    "carved-brisket-sandwich",
    ...
    "gamechanger-the-dirty-bird-2-0",
    "beef-cheesesteak",
    "big-daves-seasoned-fries",
    "cheesesteak-eggrolls",
    "molly-bs",
    "bbq-pork-burnt-ends-mac",
    "buffalo-chicken-mac",
    "cajun-crispy-chicken-sandwich",
    "berry-blast-funnel-cake-fries",
    "cheesecake-and-brownie-stacker",
    "baja-shrimp-dos-tacos",
    "carne-asada-fries"
  ]);
  ...
  console.log(
    dryRun
      ? "MERCEDES-BENZ STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "MERCEDES-BENZ STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${MERCEDES_BENZ_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of MERCEDES_BENZ_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickMercedesBenzStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
      ...
      for (const member of members) {
        const action = curatedHandled
          ? recommendCuratedMemberAction(
              ...
              pickMercedesBenzStadiumCuratedKeep(
                members,
                MERCEDES_BENZ_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (Atlanta / Falcons / Atlanta United specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      MERCEDES_BENZ_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  ...
  console.log(
    "  · lib/venue-menu-import/mercedes-benz-stadium.ts — curated dishes + concept-level vendor rows."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline dishes (404 Burger, Peach Cobbler Waffle Cone, Lemon Pepper Wet Bucket) kept despite name=vendor."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-mercedes-benz-stadium-menu-cleanup.ts --apply");
    return;
  }
