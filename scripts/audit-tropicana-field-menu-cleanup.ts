#!/usr/bin/env npx tsx
/**
 * Tropicana Field menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-tropicana-field-menu-cleanup.ts
 *   npm run audit:tropicana-field-menu
 *   npx tsx scripts/audit-tropicana-field-menu-cleanup.ts --apply
 */
...
import {
  buildTropicanaCuratedGroups,
  classifyTropicanaFieldRow,
  isTropicanaVendorStandPlaceholder,
  TROPICANA_CURATED_CLEANUP_GROUPS,
  TROPICANA_FIELD_VENUE_SLUG,
  recommendCuratedMemberAction,
  type TropicanaCleanupAction
} from "../lib/tropicana-field-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: TropicanaCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildTropicanaCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildTropicanaCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: TROPICANA_FIELD_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${TROPICANA_FIELD_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildTropicanaCuratedDuplicateGroups(items);

  const vendorOnlyRows = active.filter((item) => isTropicanaVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyTropicanaFieldRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyTropicanaFieldRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyTropicanaFieldRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: TropicanaCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyTropicanaFieldRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    for (const member of group.members) {
      const spec = TROPICANA_CURATED_CLEANUP_GROUPS.find((s) => s.id === group.key)!;
  ...
  console.log(
    dryRun ? "TROPICANA FIELD MENU CLEANUP AUDIT (DRY RUN)" : "TROPICANA FIELD MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${TROPICANA_FIELD_VENUE_SLUG})`);
  ...
  console.log(
    "  KEEP: Oreo Brownie Milkshake (trop-oreo-brownie-milkshake) — Epic Milkshakes stand label."
  );
  ...
  console.log(
    "  · lib/venue-menu-import/tropicana-field.ts imports vendor concepts as FoodItem names."
  );
  console.log(
    "  · Expansion seed (lib/mlb-ballpark-concessions-parks-wave-3.ts) adds named dishes."
  );
  console.log("  · Cubano Dog, Oreo Brownie Milkshake, and seafood items kept despite simple names.");
  console.log("  · Crumbl Cookies has no named dish at this venue — hide stand label only.");

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-tropicana-field-menu-cleanup.ts --apply");
    return;
  }
