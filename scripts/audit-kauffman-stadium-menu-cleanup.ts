#!/usr/bin/env npx tsx
/**
 * Kauffman Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-kauffman-stadium-menu-cleanup.ts
 *   npm run audit:kauffman-stadium-menu
 *   npx tsx scripts/audit-kauffman-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildKauffmanCuratedGroups,
  classifyKauffmanStadiumRow,
  isKauffmanVendorStandPlaceholder,
  KAUFFMAN_CURATED_CLEANUP_GROUPS,
  KAUFFMAN_STADIUM_VENUE_SLUG,
  recommendCuratedMemberAction,
  type KauffmanCleanupAction
} from "../lib/kauffman-stadium-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: KauffmanCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildKauffmanCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildKauffmanCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: KAUFFMAN_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${KAUFFMAN_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildKauffmanCuratedDuplicateGroups(items);

  const vendorOnlyRows = active.filter((item) => isKauffmanVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyKauffmanStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyKauffmanStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyKauffmanStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: KauffmanCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyKauffmanStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    for (const member of group.members) {
      const spec = KAUFFMAN_CURATED_CLEANUP_GROUPS.find((s) => s.id === group.key)!;
  ...
  console.log(
    dryRun
      ? "KAUFFMAN STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "KAUFFMAN STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${KAUFFMAN_STADIUM_VENUE_SLUG})`);
  ...
  if (KAUFFMAN_CURATED_CLEANUP_GROUPS.length === 0) {
    console.log("  (none — import vs seed duplicates handled via slug heuristics)");
  }
  ...
  console.log(
    "  KEEP (canonical): Hot Dog Wellington, S’mores Quesadilla, Tiramisu Helmet, Brisketacho, Dirty Soda."
  );
  ...
  console.log(
    "  · lib/venue-menu-import/kauffman-stadium.ts uses vendorName = item name for stand concepts."
  );
  console.log(
    "  · Wave-6 seed (lib/mlb-ballpark-concessions-parks-wave6.ts) adds named dishes; hide import stand labels."
  );
  console.log("  · Dirty Soda kept despite broad generic heuristic (named Fan Deck-style product).");

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-kauffman-stadium-menu-cleanup.ts --apply");
    return;
  }
