#!/usr/bin/env npx tsx
/**
 * World Cup host-venue conservative generic menu cleanup (dry-run + optional apply).
 *
 * Hides only explicit generic / value / category rows across World Cup venues.
 * Does not hide duplicate groups or rows with real reviews/photos.
 *
 *   npm run audit:world-cup-generic-menu-cleanup
 *   npx tsx scripts/audit-world-cup-generic-menu-cleanup.ts --apply
 */
...
import {
  classifyWorldCupGenericRow,
  type WorldCupGenericCleanupAction,
  type WorldCupGenericMenuItem
} from "../lib/world-cup-generic-menu-cleanup";
import {
...
type ItemRow = WorldCupGenericMenuItem & {
  itemType: ItemType;
  category: string;
...
type ClassifiedRow = {
  ...
  venueName: string;
  item: ItemRow;
  action: WorldCupGenericCleanupAction;
  reason: string;
};
...
async function main() {
  ...
  const worldCupVenues = venues.filter((v) =>
    getActiveVenueAuditGroups(v).includes("world-cup")
  );

  console.log("═".repeat(72));
  console.log(
    dryRun
      ? "WORLD CUP GENERIC MENU CLEANUP AUDIT (DRY RUN)"
      : "WORLD CUP GENERIC MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`World Cup host venues: ${worldCupVenues.length}`);
  console.log(
  ...
  for (const venue of worldCupVenues) {
    ...
    for (const item of items) {
      const { action, reason } = classifyWorldCupGenericRow(item, {
  ...
  const skippedDuplicateGeneric = skippedDuplicate.filter((r) =>
    classifyWorldCupGenericRow(r.item, { inDuplicateGroup: false }).action === "hide-generic"
  );
  ...
  if (hideRows.length === 0) {
    ...
    for (const row of hideRows) {
      if (row.venueSlug !== currentVenue) {
        ...
        const venueRecord = worldCupVenues.find((v) => v.slug === row.venueSlug);
        const groupLabel = formatAuditGroupLabel(
          getActiveVenueAuditGroups(
            venueRecord ?? { slug: row.venueSlug, leagues: [] }
          )
        );
  ...
  console.log("\n── Summary ─────────────────────────────────────────────────────────");
  console.log(`World Cup venues audited: ${worldCupVenues.length}`);
  ...
  if (dryRun) {
    console.log(
      "\nRun apply: npx tsx scripts/audit-world-cup-generic-menu-cleanup.ts --apply"
    );
    return;
  }
