/**
 * Upsert curated 2026 Big 12 football home games.
 *
 * Usage:
 *   npm run sync:ncaa-big-12-schedule
 *   npm run sync:ncaa-big-12-schedule -- --apply
 */
...
import {
  ...
  ncaaBig12MappedGameToPrismaData
} from "../lib/schedules/ncaa-big-12-schedule";
import { NCAA_BIG_12_2026_EXCLUDED_NEUTRALS } from "../lib/schedules/ncaa-big-12-2026-data";
...
async function main() {
  ...
  const mapped = mapAllBig122026HomeGames();
  ...
  console.log(`\n  NCAA Big 12 2026 schedule sync`);
  console.log(`  Mode: ${apply ? "APPLY" : "DRY RUN"}\n`);
  console.log(`  Fixtures mapped: ${mapped.length}`);
  console.log(`  Excluded neutrals: ${NCAA_BIG_12_2026_EXCLUDED_NEUTRALS.length}`);
  for (const n of NCAA_BIG_12_2026_EXCLUDED_NEUTRALS) {
  ...
  for (const game of mapped) {
    ...
    const data = ncaaBig12MappedGameToPrismaData(game, venueId);
    if (apply) {
