#!/usr/bin/env npx tsx
/**
 * Globe Life Field menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-globe-life-field-menu-cleanup.ts
 *   npm run audit:globe-life-field-menu
 *   npx tsx scripts/audit-globe-life-field-menu-cleanup.ts --apply
 */
...
import {
  buildGlobeLifeCuratedGroups,
  classifyGlobeLifeFieldRow,
  GLOBE_LIFE_CURATED_CLEANUP_GROUPS,
  GLOBE_LIFE_FIELD_VENUE_SLUG,
  isGlobeLifeVendorStandPlaceholder,
  recommendCuratedMemberAction,
  type GlobeLifeCleanupAction
} from "../lib/globe-life-field-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: GlobeLifeCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildGlobeLifeCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildGlobeLifeCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: GLOBE_LIFE_FIELD_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${GLOBE_LIFE_FIELD_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildGlobeLifeCuratedDuplicateGroups(items);

  const vendorOnlyRows = active.filter((item) => isGlobeLifeVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyGlobeLifeFieldRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyGlobeLifeFieldRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyGlobeLifeFieldRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: GlobeLifeCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyGlobeLifeFieldRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    for (const member of group.members) {
      const spec = GLOBE_LIFE_CURATED_CLEANUP_GROUPS.find((s) => s.id === group.key)!;
  ...
  console.log(
    dryRun
      ? "GLOBE LIFE FIELD MENU CLEANUP AUDIT (DRY RUN)"
      : "GLOBE LIFE FIELD MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${GLOBE_LIFE_FIELD_VENUE_SLUG})`);
  ...
  console.log(
    "  KEEP (canonical): 9th Inning Rally Sombrero, Chicken Tender Helmet, High Steaks Nachos, Texas Ribeye Sandwich, Whataburger, Dirty Soda."
  );
  ...
  console.log(
    "  · lib/venue-menu-import/globe-life-field.ts PDF import adds General Concessions category rows."
  );
  console.log(
    "  · Wave-4 seed adds named Texas/Rangers items; hide stand labels and generic hub rows."
  );
  console.log("  · Dirty Soda and Whataburger kept despite name=vendor broad-audit flags.");

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-globe-life-field-menu-cleanup.ts --apply");
    return;
  }
