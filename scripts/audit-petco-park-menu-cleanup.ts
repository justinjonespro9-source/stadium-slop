#!/usr/bin/env npx tsx
/**
 * Petco Park menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-petco-park-menu-cleanup.ts
 *   npm run audit:petco-park-menu
 *   npx tsx scripts/audit-petco-park-menu-cleanup.ts --apply
 */
...
import {
  buildPetcoCuratedGroups,
  classifyPetcoParkRow,
  isPetcoVendorStandPlaceholder,
  PETCO_CURATED_CLEANUP_GROUPS,
  PETCO_PARK_VENUE_SLUG,
  recommendCuratedMemberAction,
  type PetcoCleanupAction
} from "../lib/petco-park-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: PetcoCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildPetcoCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildPetcoCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: PETCO_PARK_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${PETCO_PARK_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const curatedGroups = buildPetcoCuratedDuplicateGroups(items);

  const vendorOnlyRows = active.filter((item) => isPetcoVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyPetcoParkRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyPetcoParkRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyPetcoParkRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: PetcoCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyPetcoParkRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    for (const member of group.members) {
      const spec = PETCO_CURATED_CLEANUP_GROUPS.find((s) => s.id === group.key)!;
  ...
  console.log(
    dryRun ? "PETCO PARK MENU CLEANUP AUDIT (DRY RUN)" : "PETCO PARK MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${PETCO_PARK_VENUE_SLUG})`);
  ...
  console.log(
    "  · lib/venue-menu-import/petco-park.ts imports vendor concepts as FoodItem names."
  );
  console.log(
    "  · Expansion seed (lib/mlb-ballpark-concessions-parks-expansion.ts) adds named dishes."
  );
  console.log("  · Padre Pretzels import variants (classic/cinnamon bites) kept — only seed dupes hidden.");

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-petco-park-menu-cleanup.ts --apply");
    return;
  }
