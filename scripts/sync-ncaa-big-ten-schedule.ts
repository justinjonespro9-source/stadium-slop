/**
 * Upsert curated 2026 Big Ten football home games.
 *
 * Usage:
 *   npm run sync:ncaa-big-ten-schedule
 *   npm run sync:ncaa-big-ten-schedule -- --apply
 */
...
import {
  ...
  ncaaBigTenMappedGameToPrismaData
} from "../lib/schedules/ncaa-big-ten-schedule";
import { NCAA_BIG_TEN_2026_EXCLUDED_NEUTRALS } from "../lib/schedules/ncaa-big-ten-2026-data";
...
async function main() {
  ...
  const mapped = mapAllBigTen2026HomeGames();
  ...
  console.log(`\n  NCAA Big Ten 2026 schedule sync`);
  console.log(`  Mode: ${apply ? "APPLY" : "DRY RUN"}\n`);
  console.log(`  Fixtures mapped: ${mapped.length}`);
  console.log(`  Excluded neutrals: ${NCAA_BIG_TEN_2026_EXCLUDED_NEUTRALS.length}`);
  for (const n of NCAA_BIG_TEN_2026_EXCLUDED_NEUTRALS) {
  ...
  for (const game of mapped) {
    ...
    const data = ncaaBigTenMappedGameToPrismaData(game, venueId);
    if (apply) {
