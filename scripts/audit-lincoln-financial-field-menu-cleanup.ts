#!/usr/bin/env npx tsx
/**
 * Lincoln Financial Field menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-lincoln-financial-field-menu-cleanup.ts
 *   npm run audit:lincoln-financial-field-menu
 *   npx tsx scripts/audit-lincoln-financial-field-menu-cleanup.ts --apply
 */
...
import {
  buildLincolnFinancialFieldCuratedGroups,
  classifyLincolnFinancialFieldRow,
  getLincolnFinancialFieldKeepBothMemberIds,
  isLincolnFinancialFieldVendorStandPlaceholder,
  LINCOLN_FINANCIAL_FIELD_CANONICAL_STAND_DISH_SLUGS,
  LINCOLN_FINANCIAL_FIELD_CURATED_CLEANUP_GROUPS,
  LINCOLN_FINANCIAL_FIELD_VENUE_SLUG,
  pickLincolnFinancialFieldCuratedKeep,
  recommendCuratedMemberAction,
  type LincolnFinancialFieldCleanupAction
} from "../lib/lincoln-financial-field-menu-cleanup";
import {
...
function formatItemLine(item: ItemRow, action: LincolnFinancialFieldCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildLincolnFinancialFieldCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildLincolnFinancialFieldCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: LINCOLN_FINANCIAL_FIELD_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${LINCOLN_FINANCIAL_FIELD_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getLincolnFinancialFieldKeepBothMemberIds(active);
  ...
  const curatedGroups = buildLincolnFinancialFieldCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isLincolnFinancialFieldVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifyLincolnFinancialFieldRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifyLincolnFinancialFieldRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifyLincolnFinancialFieldRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: LincolnFinancialFieldCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifyLincolnFinancialFieldRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickLincolnFinancialFieldCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickLincolnFinancialFieldCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    ...
    "philly-cheesesteak",
    "cheesesteak",
    "america-s-pie-the-vincent",
    "g-day-gourmet-maui-bbq-meat-pie",
    "the-pierogie-place-kielbasa-pierogie-special",
    ...
    "bone-in-wings",
    "boneless-wings",
    "carmichael-bbq-chicken-sandwich",
    "vegan-burrito",
    "vegetarian-burger",
    "kosher-hot-dog",
    "gluten-free-hot-dog",
  ...
  const specialtyDrinkRows = active.filter((item) =>
    /south philly caipirinha|red faithful margarita|stateside vodka/i.test(
      `${item.name} ${item.slug}`
    )
  );
  ...
  console.log(
    dryRun
      ? "LINCOLN FINANCIAL FIELD MENU CLEANUP AUDIT (DRY RUN)"
      : "LINCOLN FINANCIAL FIELD MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${LINCOLN_FINANCIAL_FIELD_VENUE_SLUG})`);
  ...
  for (const spec of LINCOLN_FINANCIAL_FIELD_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickLincolnFinancialFieldCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickLincolnFinancialFieldCuratedKeep(
                    members,
                    LINCOLN_FINANCIAL_FIELD_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (Philadelphia / Eagles / World Cup specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      LINCOLN_FINANCIAL_FIELD_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (South Philly Caipirinha) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Named cocktail headline row (South Philly Caipirinha) is kept as a canonical stand dish."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/lincoln-financial-field.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline cocktail (South Philly Caipirinha) kept despite name=vendor."
  );
  console.log(
    "  · Kelly Green Jumbo Cookie — keep Sideline Markets import; hide NFL headline duplicate row."
  );
  console.log(
    "  · Hide stand placeholders (Chickie's, Tony Luke's, Philip's, etc.) where named menu items exist; hide Xfinity Live! NFL junk row."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-lincoln-financial-field-menu-cleanup.ts --apply");
    return;
  }
