#!/usr/bin/env npx tsx
/**
 * SoFi Stadium menu cleanup audit (dry-run + optional apply).
 *
 *   npx tsx scripts/audit-sofi-stadium-menu-cleanup.ts
 *   npm run audit:sofi-stadium-menu
 *   npx tsx scripts/audit-sofi-stadium-menu-cleanup.ts --apply
 */
...
import {
  buildSofiStadiumCuratedGroups,
  classifySofiStadiumRow,
  getSofiStadiumKeepBothMemberIds,
  isSofiStadiumVendorStandPlaceholder,
  SOFI_STADIUM_CANONICAL_STAND_DISH_SLUGS,
  SOFI_STADIUM_CURATED_CLEANUP_GROUPS,
  SOFI_STADIUM_VENUE_SLUG,
  pickSofiStadiumCuratedKeep,
  recommendCuratedMemberAction,
  type SofiStadiumCleanupAction
} from "../lib/sofi-stadium-menu-cleanup";
import {
...
const SOURCE_URL =
  "https://dining.hollywoodparkca.com";
...
function formatItemLine(item: ItemRow, action: SofiStadiumCleanupAction, reason: string) {
  return [
    `  [${action}]`,
    `"${item.name}"`,
    `slug=${item.slug}`,
...
function buildSofiStadiumCuratedDuplicateGroups(items: ItemRow[]) {
  const active = items.filter((i) => i.status === EntityStatus.ACTIVE);
  return buildSofiStadiumCuratedGroups(active).map(({ spec, members }) => ({
...
async function main() {
  ...
  const venue = await prisma.venue.findFirst({
    where: { slug: { equals: SOFI_STADIUM_VENUE_SLUG, mode: "insensitive" } }
  });
  if (!venue) {
    console.error(`Venue not found: ${SOFI_STADIUM_VENUE_SLUG}`);
    process.exit(1);
  }
  ...
  const keepBothMemberIds = getSofiStadiumKeepBothMemberIds(active);
  ...
  const curatedGroups = buildSofiStadiumCuratedDuplicateGroups(items);
  const curatedMemberIds = new Set(
    curatedGroups.flatMap((g) => g.members.map((m) => m.id))
  );

  const vendorOnlyRows = active.filter((item) => isSofiStadiumVendorStandPlaceholder(item));
  const genericConcessionRows = active.filter(
    (item) => classifySofiStadiumRow(item).kind === "generic-concession"
  );
  const genericBeverageRows = active.filter(
    (item) => classifySofiStadiumRow(item).kind === "generic-beverage"
  );
  const vagueRows = active.filter((item) => classifySofiStadiumRow(item).kind === "vague");
  ...
  const hidePlan = new Map<
    string,
    { item: ItemRow; action: SofiStadiumCleanupAction; reason: string; keep?: ItemRow }
  >();
  const manualRows: ItemRow[] = [];

  for (const item of active) {
    const classified = classifySofiStadiumRow(item);
  ...
  for (const group of curatedGroups) {
    ...
    const keep = pickSofiStadiumCuratedKeep(group.members, group.spec);
    ...
    for (const member of members) {
      ...
      if (curatedMemberIds.has(member.id) && hidePlan.has(member.id) === false) {
        const curatedKeep = curatedGroups.some((g) =>
          g.members.some((m) => m.id === member.id && m.id === pickSofiStadiumCuratedKeep(g.members, g.spec).id)
        );
  ...
  const notableKeepSlugs = new Set([
    ...
    "al-pastor-torta",
    "la-street-dog",
    "san-vicente-blvd-loaded-beef-shawarma-fries",
    "sawtelle-blvd-tempura-stuffed-peppers",
    "beef-shawarma-fries",
    "teriyaki-fries",
    "cheeseburger",
    "burger-of-the-month",
  ...
  const specialtyDrinkRows = active.filter(
    (item) =>
      /dirty hollywood soda/i.test(`${item.name} ${item.slug}`) &&
  ...
  console.log(
    dryRun
      ? "SOFI STADIUM MENU CLEANUP AUDIT (DRY RUN)"
      : "SOFI STADIUM MENU CLEANUP (APPLY)"
  );
  console.log("═".repeat(72));
  console.log(`Venue: ${venue.name} (${SOFI_STADIUM_VENUE_SLUG})`);
  ...
  for (const spec of SOFI_STADIUM_CURATED_CLEANUP_GROUPS) {
    ...
    const keep =
      members.length >= 2 && spec.treatAsDuplicate
        ? pickSofiStadiumCuratedKeep(members, spec)
        : null;
    for (const member of members) {
      const action =
        members.length < 2
          ...
          keepBothMemberIds.has(member.id)
            ...
            : curatedHandled
              ? recommendCuratedMemberAction(
                  ...
                  pickSofiStadiumCuratedKeep(
                    members,
                    SOFI_STADIUM_CURATED_CLEANUP_GROUPS.find((s) =>
  ...
  console.log("\n── Notable keep items (Los Angeles / Rams / Chargers / World Cup specialties) ────────────");
  const notableKeep = active.filter(
    (item) =>
      notableKeepSlugs.has(item.slug) ||
      SOFI_STADIUM_CANONICAL_STAND_DISH_SLUGS.has(item.slug)
  );
  for (const item of notableKeep) {
    console.log(`    · ${item.name} (${item.slug}) — ${item.vendor.name}`);
  }

  console.log("\n── 21+ specialty drinks (Dirty Hollywood Soda) ───────");
  if (specialtyDrinkRows.length === 0) {
    console.log(
      "  Named cocktail headline row (Dirty Hollywood Soda) is kept as a canonical stand dish."
    );
  } else {
  ...
  console.log(
    "  · lib/venue-menu-import/sofi-stadium.ts — curated dishes; beverage/stand rows intentionally skipped in curation."
  );
  console.log(
    "  · data/league-import/nfl-venues-import.cleaned.csv — NFL headline/partner stand imports."
  );
  console.log(
    "  · Headline items (Twinkie Burger, Char Siu Bowl, Dirty Hollywood Soda, Haupia Tart, Tuxedo Cake) kept despite name=vendor where applicable."
  );
  console.log(
    "  · Loaded Mac & Cheese Bowl — hide generic Mac & Cheese Bowl at BBQ; Twinkie Burger — keep BBQ import, hide NFL headline row."
  );
  console.log(
    "  · Hide stand placeholders (Fairfax Ave, Olvera Street, Sawtelle Blvd, etc.) and NFL junk rows (Uber Eats, Zippin awareness)."
  );

  if (dryRun) {
    console.log("\nRun apply: npx tsx scripts/audit-sofi-stadium-menu-cleanup.ts --apply");
    return;
  }
