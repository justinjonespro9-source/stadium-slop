...
export function FoodItemAgeGate({ alcoholRelated, children }: FoodItemAgeGateProps) {
  ...
  return (
    <div className="relative mt-3">
      {status === "declined" || (status === "unknown" && showPrompt) ? (
        <AgeConfirmationPrompt className="mb-3" />
      ) : null}
      <div
      ...
      <div className="absolute inset-0 z-10 flex items-start justify-center px-2 pt-8 sm:px-4">
        <div className="max-w-md rounded-xl border border-[var(--slop-line-strong)] bg-[color:rgba(6,15,24,0.92)] px-4 py-3 text-center shadow-[0_12px_40px_rgba(0,0,0,0.5)]">
          <p className="text-sm font-black text-[var(--slop-cream)]">
            {ALCOHOL_HIDDEN_MESSAGE}
          </p>
          {status === "declined" ? (
            <p className="mt-2 text-[0.7rem] leading-snug text-[var(--slop-cream-dim)] sm:text-xs">
